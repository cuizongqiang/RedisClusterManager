 ...
 */
    ...
    for (var i = 0, n = types.length; i < n; i++) {
        (function (type) {
            app.directive(type + 'Chart', [
                '$http',
                'theme',
                'util',
                function ($http, theme, util) {
                    return {
                        restrict: 'EA',
 ...
 */
    angular.module('angular-echarts.util', []).factory('util', function () {
            ...
            function isGraphChart(type){
                return ['graph'].indexOf(type) > -1;
            }
            function getAxisTicks(data, config, type) {
                var ticks = [];
                if (data[0] != null) {
                    angular.forEach(data[0].datapoints, function (datapoint) { ticks.push(datapoint.x); });
                }
                return {
                    type: 'category',
            ...
            function getLegend(data, config, type) {
                ...
                if (isPieChart(type)) {
                    if (data[0]) {
                        angular
                            .forEach(data[0].datapoints, function (datapoint) {
                                legend
                                    .data
                                    .push(datapoint.x);
                            });
     ...
     */
            function getTooltip(data, config, type) {
                ...
                return angular.extend(tooltip, angular.isObject(config.tooltip)
                    ? config.tooltip
                    : {});
            }
 ...
 */
    angular
        ...
        .factory('theme', [
            ...
            'dark',
            'roma',
            function (infographic, macarons, shine, dark, roma) {
                var themes = {
